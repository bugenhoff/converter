...
17. ✅ **Groq-Only + Adaptive Request Splitting** — добавлен режим `PDF_CONVERSION_MODE` (включая `groq_only`), реализована адаптивная стратегия Groq (split/downscale/retry + лимит запросов на документ, запрет partial output), обновлены тесты и документация.
18. ✅ **Groq Text-Preservation Mode** — Groq-промпт упрощен до постраничной транскрипции (`page_number + text`), добавлена строгая JSON-нормализация/валидация полноты и сборка DOCX как редактируемого текста с page break по страницам; дефолты смещены в сторону полноты (`GROQ_BATCH_SIZE=1`, `GROQ_MAX_TOKENS=12000`).
